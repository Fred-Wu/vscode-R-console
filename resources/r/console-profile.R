...
# Connect this R session through the integration selected by the extension.
local({
    session_bootstrap <- Sys.getenv("R_CONSOLE_SESSION_BOOTSTRAP")
    if (nzchar(session_bootstrap) && file.exists(session_bootstrap)) {
        source(session_bootstrap, local = TRUE)
    }
})
